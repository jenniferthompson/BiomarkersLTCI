...
We present descriptive statistics and data visualizations for the `r length(biomarker.list)` biomarkers collected in both the BRAIN-ICU and MIND-ICU cohorts. For speed and ease, only BRAIN-ICU data is included here, assuming that patterns will be similar for MIND-ICU.
...
## -- Function for summary statistic histograms -- ##
bio.hist <- function(usedata, xvar, xlabel, resultstr, resultx){
  ...
  ggplot(aes(x = xvar), data = usedata) +
    ...
    annotate(geom = 'text', label = as.character(resultstr), x = resultx, y = Inf, vjust = 1.5, hjust = 1, size = 2.5) +
    theme_minimal() +
...
bio.plots <- function(use.bio){
  ...
  bio.hist <- ggplot(aes(x = use.bio), data = use.daily) +
    geom_histogram(alpha = 0.3, fill = 'blue') +
    scale_x_continuous(name = 'Biomarker Value, All Study Days') +
    scale_y_continuous(name = 'Frequency') +
    annotate(geom = 'text', label = use.desc$n.pct, x = max(use.daily$use.bio, na.rm = TRUE), y = Inf, vjust = 1.3, hjust = 1, size = 2.5) +
  ...
  bio.mean.hist <- bio.hist(usedata = use.summary, 'pt.mean', 'Patient Mean', use.desc$mean.desc, max(use.summary$pt.mean, na.rm = TRUE))
  bio.min.hist <- bio.hist(usedata = use.summary, 'pt.min', 'Patient Minimum', use.desc$min.desc, max(use.summary$pt.min, na.rm = TRUE))
  bio.max.hist <- bio.hist(usedata = use.summary, 'pt.max', 'Patient Maximum', use.desc$max.desc, max(use.summary$pt.max, na.rm = TRUE))
  bio.delta.hist <- bio.hist(usedata = use.summary, 'pt.delta', 'Patient Change, Last - First', use.desc$delta.desc, max(use.summary$pt.delta, na.rm = TRUE))
  bio.auc.hist <- bio.hist(usedata = use.summary, 'pt.auc', 'Patient AUC', use.desc$auc.desc, max(use.summary$pt.auc, na.rm = TRUE))
  
  return(list('total.hist' = bio.hist,
...
## All combinations of biomarkers
...
marker.comb.unique <- data.frame(marker1 = unlist(lapply(markercomb.strsplit, FUN = function(x){ x[[1]][1] })),
...
corrplotmat[upper.tri(corrplotmat)] <- (sum(lower.tri(corrplotmat)) + 1):(sum(lower.tri(corrplotmat))*2)
diag(corrplotmat) <- (sum(!is.na(corrplotmat)) + 1):length(corrplotmat)

## Function to create scatterplot with smoother
...
## Function to create "plot" with rho, p-value
rhop <- function(xvar, yvar){
  if(xvar == yvar){
    ...
    ggplot(aes(x = xvar, y = yvar), data = usedata) +
      annotate(geom = 'text', x = mean(usedata$xvar, na.rm = TRUE), y = mean(usedata$yvar, na.rm = TRUE), label = corrstr, size = 3) +
      theme_minimal() +
...
bio.plots <- lapply(biomarker.list, FUN = function(marker){
  ggplot(aes(x = xval, y = yval), data = bio.plot.fakedata) +
      annotate(geom = 'text', x = 5.5, y = 5.5, label = toupper(marker), face = 'bold', colour = 'navy', size = 3.7) +
      theme_minimal() +
      theme(axis.title = element_blank(),
            axis.text = element_blank(),
            axis.ticks = element_blank(),
