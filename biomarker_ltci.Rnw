...
bio.summary.wide <- bio.summary.pt %>%
  gather(key = 'measure', value = 'quantity', pt.n:pt.delta) %>%
...
## 1. Logistic regression, using "in cohort" as outcome
## 2. Get estimated probability for each person in cohort
## 3. Weight = 1 / estimated probability

## Subset data to remove observations with missing values; makes merging easier
use.for.weights <- all.oneobs %>%
  left_join(bio.summary.wide[,c('id', gsub('\\.imp$', '\\.mean', biomarker.vnames))], by = 'id') %>%
  filter(!is.na(age.enroll) & !is.na(charlson.score) & !is.na(edu) & !is.na(iqcode.score.e) &
           !is.na(stroke.risk) & !is.na(mean.benz.cube) & !is.na(mean.op.cube) &
           !is.na(mean.prop.cube) & !is.na(mean.dex.cube) & !is.na(mean.hal.cube) &
           !is.na(icudays.sevseptic.s) & !is.na(del.s.imp) & !is.na(coma.s.imp) &
           !is.na(mean.modsofa.icu) & !is.na(cog.cohort.3 ) & !is.na(cog.cohort.12) &
           !is.na(func.cohort.3) & !is.na(func.cohort.12) & !is.na(crp.mean) &
           !is.na(ifngamma.mean) & !is.na(il10.mean) & !is.na(il12.mean) & !is.na(il1b.mean) &
           !is.na(il6.mean) & !is.na(il8.mean) & !is.na(mmp9.mean) & !is.na(proteinc.mean) &
           !is.na(tnfa.mean) & !is.na(tnfr1.mean))

## Function to fit "in cohort ~ covariates" model for each cohort ##
prob.cohort <- function(cohort.var){
  fitted(glm(as.formula(paste(cohort.var,
                              "rcs(age.enroll, 3) + rcs(charlson.score, 3) + edu +
                               ...
                               rcs(il6.mean, 3) + rcs(il8.mean) + rcs(mmp9.mean, 3) +
                               rcs(proteinc.mean, 3) + rcs(tnfa.mean, 3) + rcs(tnfr1.mean, 3)",
                              sep = ' ~ ')),
             family = 'binomial',
             data = use.for.weights))
}

use.for.weights$prob.cog.3 <- prob.cohort('cog.cohort.3')
use.for.weights$prob.cog.12 <- prob.cohort('cog.cohort.12')
use.for.weights$prob.func.3 <- prob.cohort('func.cohort.3')
use.for.weights$prob.func.12 <- prob.cohort('func.cohort.12')
use.for.weights$wt.cog.3 <- 1 / use.for.weights$prob.cog.3
use.for.weights$wt.cog.12 <- 1 / use.for.weights$prob.cog.12
use.for.weights$wt.func.3 <- 1 / use.for.weights$prob.func.3
use.for.weights$wt.func.12 <- 1 / use.for.weights$prob.func.12

## Checks for weights
# sum(subset(use.for.weights, cog.cohort.3)$wt.cog.3)
# sum(subset(use.for.weights, cog.cohort.12)$wt.cog.12)
# sum(subset(use.for.weights, func.cohort.3)$wt.func.3)
# sum(subset(use.for.weights, func.cohort.12)$wt.func.12)

# sum(round(subset(use.for.weights, cog.cohort.3)$wt.cog.3, 1))
# sum(round(subset(use.for.weights, cog.cohort.12)$wt.cog.12, 1))
# sum(round(subset(use.for.weights, func.cohort.3)$wt.func.3, 1))
# sum(round(subset(use.for.weights, func.cohort.12)$wt.func.12, 1))
# 
# ## Check using plots
# ggplot(aes(x = cog.cohort.12, y = prob.cog.12), data = use.for.weights) + geom_boxplot() + scale_y_continuous(limits = 0:1)
...
model.data <- subset(all.oneobs,
                     select = c(id, age.enroll, charlson.score, edu, iqcode.score.e, stroke.risk,
                                ...
                                mean.hal.cube, adl.e, faq.e, frailty, cog.cohort.3,
                                cog.cohort.12, func.cohort.3, func.cohort.12)) %>%
  left_join(select(use.for.weights, id, wt.cog.3, wt.cog.12, wt.func.3, wt.func.12), by = 'id') %>%
