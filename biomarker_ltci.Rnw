...
n.nobio <- as.numeric(hosp.table['Survived hospital, but no biomarker measured'])
n.elig.3 <- as.numeric(hosp.table['Survived with >=1 biomarker measured'])
...
consort.df$rowlab <- c('Enrolled', '~~~Withdrew, requested data be destroyed',
                       ...
                       '~~~~~~Survived, but no biomarker measured',
...
desc.data.all$cohort <- factor(desc.data.all$cohort,
                               ...
                               labels = c('Enrolled Patients with Biomarkers',
                                          'Patients with >=1 Biomarker + Long-Term Outcomes'))
...
bio.values <- all.daily[,biomarker.vnames]
bio.values.longterm <-
  subset(all.daily,
