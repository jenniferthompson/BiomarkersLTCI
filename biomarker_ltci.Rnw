...
\usepackage[margin=.35in]{geometry} %resets margins
\usepackage{hyperref}
\usepackage{lscape}
\usepackage{pdfpages}
...
biomarker.vnames <- paste0(
  c('crp', 'ifngamma', 'il10', 'il12', 'il1b', 'il6', 'il8', 'mmp9', 'proteinc',
    'tnfa', 'tnfr1'),
  '.imp'
)

## Load all BRAIN+MINDICU data
if(Sys.info()['sysname'] == 'Darwin'){
  if(length(grep('Volumes', getwd())) > 0){
    load('/Volumes/thomps23/ICUDelirium/BRAINMIND/CombineData/analysisdata/brainmind_deid.Rdata')
    source('/Volumes/thomps23/R/multiplot.r')
  } else{
    load('brainmind_deid.Rdata')
    source('multiplot.r')
  }
} else{
  load('/home/thomps23/ICUDelirium/BRAINMIND/CombineData/analysisdata/brainmind_deid.Rdata')
  source('/home/thomps23/R/multiplot.r')
}

## Read in cohort indicators (specifically need biomarker, followup indicators)
inhosp_ind <- readRDS("../../BRAINMIND/CombineData/cohorttables/inhosp_cohorts.rds")
status_ind <- readRDS("../../BRAINMIND/CombineData/cohorttables/overall_status.rds")
fu_ind <- readRDS("../../BRAINMIND/CombineData/cohorttables/fu_cohorts.rds")

## Create cube root versions of all mean 24h ICU drug variables
brainmind.oneobs$mean.benz.cube <- brainmind.oneobs$mean.benz.icu^(1/3)
brainmind.oneobs$mean.op.cube <- brainmind.oneobs$mean.op.icu^(1/3)
brainmind.oneobs$mean.prop.cube <- brainmind.oneobs$mean.prop.icu^(1/3)
brainmind.oneobs$mean.dex.cube <- brainmind.oneobs$mean.dex.icu^(1/3)
brainmind.oneobs$mean.hal.cube <- brainmind.oneobs$mean.hal.icu^(1/3)

label(brainmind.oneobs$mean.benz.cube) <- 'Mean 24h benzos in ICU, cube root'
label(brainmind.oneobs$mean.op.cube) <- 'Mean 24h opioids in ICU, cube root'
label(brainmind.oneobs$mean.prop.cube) <- 'Mean 24h propofol in ICU, cube root'
label(brainmind.oneobs$mean.dex.cube) <- 'Mean 24h dex in ICU, cube root'
label(brainmind.oneobs$mean.hal.cube) <- 'Mean 24h haldol in ICU, cube root'

@

The following analyses examine the associations between a panel of \Sexpr{length(biomarker.vnames)} biomarkers measured in the BRAIN-ICU and MIND-ICU cohorts
and long-term cognitive and functional outcomes.

\section{Cohort Definitions}
Cohorts are defined separately for each time point (3 and 12 months) and for
cognitive and functional outcomes, since patients could have at least some
functional testing done by phone. To be included in any analyses, patients must
have had at least one biomarker measured in the ICU. Patients are included in
the cognitive cohort if they had at least one cognitive test performed at the
specified time point (including any RBANS domains, Trails A, and/or Trails B).
Patients are included in the functional cohort if they had at least one
functional test performed at the specified time point (BDI-II, PCL, ADL, FAQ,
SF36 mental or physical component, and/or CSHA frailty).

<<datamgmt>>=
## -- Indicator for whether patient has any biomarkers measured on any study day -------------------
brainmind.daily$had.all.biomarkers <-
  rowSums(!is.na(brainmind.daily[,biomarker.vnames])) == length(biomarker.vnames)
brainmind.oneobs$had.all.biomarkers <- brainmind.oneobs$id %in% subset(brainmind.daily, had.all.biomarkers)$id
...
brainmind.fu$has.cog.data <- rowSums(!is.na(brainmind.fu[,cog.varnames])) > 0
brainmind.fu$has.func.data <- rowSums(!is.na(brainmind.fu[,func.varnames])) > 0

## Indicators for complete vs. partial data
brainmind.fu$complete.cog <- ifelse(!brainmind.fu$has.cog.data, NA,
                              rowSums(!is.na(brainmind.fu[,cog.varnames])) == length(cog.varnames))
brainmind.fu$complete.func <- ifelse(!brainmind.fu$has.func.data, NA,
                               rowSums(!is.na(brainmind.fu[,func.varnames])) == length(func.varnames))

## Create indicators for whether patient is in cohort at each time
## NOTE: 14050 is recorded as withdrawing in the hospital, but has followup data;
## We are taking the conservative approach of not using this patient's f/u data.
brainmind.oneobs$cog.cohort.3 <- with(brainmind.oneobs, {
  had.all.biomarkers &
    id %in% subset(brainmind.fu, fu.period == '3 Month' & has.cog.data)$id &
    !(id == 14050)
})
brainmind.oneobs$cog.cohort.12 <- with(brainmind.oneobs, {
  had.all.biomarkers &
    id %in% subset(brainmind.fu, fu.period == '12 Month' & has.cog.data)$id &
    !(id == 14050)
})
brainmind.oneobs$func.cohort.3 <- with(brainmind.oneobs, {
  had.all.biomarkers &
    id %in% subset(brainmind.fu, fu.period == '3 Month' & has.func.data)$id &
    !(id == 14050)
})
brainmind.oneobs$func.cohort.12 <- with(brainmind.oneobs, {
  had.all.biomarkers &
    id %in% subset(brainmind.fu, fu.period == '12 Month' & has.func.data)$id &
    !(id == 14050)
})

## Create indicators for whether patient is in each cohort at either time, for Table 1
brainmind.oneobs$cog.cohort.either <- brainmind.oneobs$cog.cohort.3 | brainmind.oneobs$cog.cohort.12
brainmind.oneobs$func.cohort.either <- brainmind.oneobs$func.cohort.3 | brainmind.oneobs$func.cohort.12

## -- Calculate summary values for each biomarker for each patient --------------------------------
bio.summary.pt <- brainmind.daily[brainmind.daily$had.all.biomarkers
...
model.data <- subset(
  ...
  had.all.biomarkers,
  select = c(
    id, age.enroll, sex.pp, charlson.score, edu, iqcode.score.e, stroke.risk,
    mean.benz.cube, mean.op.cube, mean.prop.cube, mean.dex.cube, mean.hal.cube,
    adl.e, faq.e, frailty, icudays.sevseptic.s, mean.modsofa.icu, del.s.imp,
    coma.s.imp, vent.los.tot.s, cog.cohort.3, cog.cohort.12, func.cohort.3,
    func.cohort.12
  )
) %>%
  left_join(bio.summary.wide, by = 'id') %>%
  ## Numeric version of frailty at enrollment
  mutate(frailty.num = as.numeric(frailty))

dd <- datadist(model.data); options(datadist = 'dd')

## Create aregImpute object to use in weighting model
set.seed(1)
areg.weight <- aregImpute(
  ~ age.enroll + charlson.score + I(edu) + I(iqcode.score.e) + I(frailty.num) +
    stroke.risk + mean.benz.cube + mean.op.cube + mean.prop.cube +
    I(mean.dex.cube) + I(mean.hal.cube) + icudays.sevseptic.s + del.s.imp +
    coma.s.imp + mean.modsofa.icu + crp.mean.t + ifngamma.mean.t + il10.mean.t +
    il12.mean.t + il1b.mean.t + il6.mean.t + il8.mean.t + mmp9.mean.t +
    proteinc.mean.t + tnfa.mean.t + tnfr1.mean.t,
  n.impute = 10,
  nk = 3,
  data = model.data
)
...
prob.cohort <- function(cohort.var){
  wtmod <- fit.mult.impute(
    formula = as.formula(
      paste(
        cohort.var,
        "rcs(age.enroll, 3) + rcs(charlson.score, 3) + edu + frailty.num +
         iqcode.score.e + rcs(stroke.risk, 3) + rcs(mean.benz.cube, 3) +
         rcs(mean.op.cube, 3) + rcs(mean.prop.cube, 3) + mean.dex.cube +
         mean.hal.cube + rcs(icudays.sevseptic.s, 3) + rcs(del.s.imp, 3) +
         rcs(coma.s.imp, 3) + rcs(mean.modsofa.icu, 3) + rcs(crp.mean.t, 3) +
         rcs(ifngamma.mean.t, 3) + rcs(il10.mean.t, 3) + rcs(il12.mean.t, 3) +
         rcs(il1b.mean.t, 3) + rcs(il6.mean.t, 3) + il8.mean.t +
         rcs(mmp9.mean.t, 3) + rcs(proteinc.mean.t, 3) + rcs(tnfa.mean.t, 3) +
         rcs(tnfr1.mean.t, 3)",
        sep = ' ~ '
      )
    ),
...
#   aes(x = cog.cohort.12, y = prob.cog.12) +
#   geom_boxplot() +
#   scale_y_continuous(limits = 0:1)

## -- Create data set with all covariates, test scores, weights ----------------
## Transpose followup scores
model.fu.data <- brainmind.fu %>%
  filter(fu.period %in% c('3 Month', '12 Month')) %>%
  mutate(
    ## Create rounded version of FAQ score - negative binomial model can't
    ##  handle non-integers created by imputing missing questions
    faq.rdscore = round(faq.totscore),
    ## Create numeric version of frailty
    frailty.fu = as.numeric(substr(frailty.fu, 1, 1))
  ) %>%
  dplyr::select(id, fu.period, rbans.global.score, trail.b.tscore, adl.totscore,
                faq.totscore, faq.rdscore, frailty.fu) %>%
  separate(fu.period, into = c('month', 'text'), sep = ' ') %>%
  gather(key = 'test', value = 'score', rbans.global.score:frailty.fu) %>%
...
label(model.data$frailty.fu.3) <- 'Clinical frailty score, 3m'
label(model.data$frailty.fu.12) <- 'Clinical frailty score, 12m'
...
We also present boxplots of each biomarker by study day, again with reference lines for the limit of detection.
...
brainmind.oneobs$hosp.status <- with(brainmind.oneobs, {
  factor(
    ifelse(
      (!is.na(studywd.amt) & studywd.amt %in% fully_withdrew) | id == 14015, 1,
      ## 14015 had all data withdrawn, but not recorded in dates form)
    ifelse(!is.na(died.inhosp) & died.inhosp == 'Died in hospital', 2,
    ifelse(!is.na(studywd.amt) | id == 14015, 3,
...
brainmind.oneobs$three.status.cog <- with(brainmind.oneobs, {
  factor(ifelse(hosp.status == 'Withdrew and requested data be destroyed', 1,
         ...
         ifelse(cog.cohort.3 & id %in% subset(brainmind.fu, fu.period == '3 Month' & complete.cog)$id, 9,
         ifelse(cog.cohort.3 & id %in% subset(brainmind.fu, fu.period == '3 Month' & !complete.cog)$id, 8,
         ifelse(id %in% subset(brainmind.fu, fu.period == '3 Month' & status == 'Deceased')$id, 5,
         ifelse(id %in% subset(brainmind.fu,
                               fu.period == '3 Month' &
...
brainmind.oneobs$three.status.func <- with(brainmind.oneobs, {
  factor(ifelse(hosp.status == 'Withdrew and requested data be destroyed', 1,
         ifelse(hosp.status == 'Died in hospital', 2,
         ifelse(hosp.status == 'Withdrew in hospital', 3,
         ifelse(hosp.status == 'Survived hospital, but no days with all biomarkers', 4,
         ifelse(func.cohort.3 & id %in% subset(brainmind.fu, fu.period == '3 Month' & complete.func)$id,
                9,
         ifelse(func.cohort.3 & id %in% subset(brainmind.fu, fu.period == '3 Month' & !complete.func)$id,
                8,
         ifelse(id %in% subset(brainmind.fu, fu.period == '3 Month' & status == 'Deceased')$id, 5,
...
brainmind.oneobs$twelve.status.cog <- with(brainmind.oneobs, {
  factor(ifelse(hosp.status == 'Withdrew and requested data be destroyed', 1,
         ...
         ifelse(hosp.status == 'Survived hospital, but no days with all biomarkers', 4,
         ifelse(cog.cohort.12 & id %in% subset(brainmind.fu, fu.period == '12 Month' & complete.cog)$id,
                9,
         ifelse(cog.cohort.12 & id %in% subset(brainmind.fu, fu.period == '12 Month' & !complete.cog)$id,
                8,
         ifelse(id %in% subset(brainmind.fu, fu.period == '12 Month' & status == 'Deceased')$id, 5,
         ifelse(id %in% subset(brainmind.fu,
                               fu.period == '12 Month' &
                                 status == 'Living-Withdrew from the study')$id,
                6,
...
brainmind.oneobs$twelve.status.func <- with(brainmind.oneobs, {
  factor(ifelse(hosp.status == 'Withdrew and requested data be destroyed', 1,
         ifelse(hosp.status == 'Died in hospital' |
         ...
         ifelse(hosp.status == 'Survived hospital, but no days with all biomarkers', 4,
         ifelse(func.cohort.12 & id %in% subset(brainmind.fu, fu.period == '12 Month' & complete.func)$id,
                9,
         ifelse(func.cohort.12 &
                  id %in% subset(brainmind.fu, fu.period == '12 Month' & !complete.func)$id,
                8,
         ifelse(id %in% subset(brainmind.fu, fu.period == '12 Month' & status == 'Deceased')$id, 5,
         ifelse(id %in% subset(brainmind.fu,
                               fu.period == '12 Month' &
...
n.enrolled <- nrow(brainmind.oneobs)

## In-hospital section
hosp.table <- table(brainmind.oneobs$hosp.status, useNA = 'ifany')
...
## 3-month section
three.cog.table <- table(brainmind.oneobs$three.status.cog, useNA = 'ifany')
three.func.table <- table(brainmind.oneobs$three.status.func, useNA = 'ifany')
...
## 12-month section
twelve.cog.table <- table(brainmind.oneobs$twelve.status.cog, useNA = 'ifany')
twelve.func.table <- table(brainmind.oneobs$twelve.status.func, useNA = 'ifany')
...
<<desctables>>=
desc.data <- brainmind.oneobs %>% left_join(model.fu.data, by = 'id')

desc.vars <- c(
  'age.enroll', 'sex.pp', 'race.pp', 'edu', 'charlson.score', 'iqcode.score.e',
  'stroke.risk', 'adl.e', 'faq.e', 'frailty', 'icu.type', 'num.apache', 'sofa',
  'mean.benz.icu', 'mean.op.icu', 'mean.prop.icu', 'mean.dex.icu',
  'mean.hal.icu', 'ever.vent.s', 'vent.los.tot.eo', 'ever.del.s.imp',
  'del.s.imp.eo', 'ever.coma.s.imp', 'coma.s.imp.eo', 'ever.sevseptic.s',
  'icudays.sevseptic.eo',
  ...
  'faq.totscore.12', 'frailty.fu.3', 'frailty.fu.12'
)

desc.data.all <-
  desc.data[desc.data$id %in%
              subset(brainmind.oneobs,
                     had.all.biomarkers &
                       !(id == 14015 |
...
label(desc.data.all$mean.op.icu) = 'Mean 24h opioids in ICU (fentanyl equivalents)'
...
label(desc.data.all$frailty.fu.3) = 'Clinical frailty score, 3m'
label(desc.data.all$frailty.fu.12) = 'Clinical frailty score, 12m'
...
bio.values <- brainmind.daily[brainmind.daily$had.all.biomarkers, c('study.day', biomarker.vnames)]
bio.values.longterm <-
  subset(brainmind.daily,
         had.all.biomarkers &
          id %in% subset(brainmind.oneobs,
                         cog.cohort.either | func.cohort.either)$id)[, c('study.day',
                                                                         biomarker.vnames)]
...
bio.summary.longterm <-
  subset(bio.summary.wide, id %in% subset(brainmind.oneobs, cog.cohort.either | func.cohort.either)$id)
...
## -- Boxplots of biomarker values by study day ----------------------------------------------------
bio.values.long <- bio.values.cohorts %>%
  filter(cohort == 'Enrolled + Biomarkers') %>%
  gather(key = biomarker, value = markerval, crp.imp:tnfr1.imp) %>%
  mutate(biomarker = toupper(gsub('\\.imp$', '', biomarker)))

bio.box.day <- ggplot(data = bio.values.long, aes(x = factor(study.day), y = markerval)) +
  geom_boxplot(fill = 'navy', colour = 'gray50') +
  facet_wrap(~ biomarker, scales = 'free_y', ncol = 3) +
  scale_x_discrete(labels = paste('ICU Day', c(1, 3, 5)), name = '') +
...
## Done differently for cognitive and functional outcomes, because mice uses
##  entire data set whereas aregImpute does not
...
always.use <- c(
  'age.enroll', 'sex.pp', 'charlson.score', 'edu', 'iqcode.score.e', 'frailty.num',
  'stroke.risk', 'mean.benz.cube', 'mean.op.cube', 'mean.prop.cube',
  'mean.dex.cube', 'mean.hal.cube', 'adl.e', 'faq.e', 'icudays.sevseptic.s',
  ...
  'trail.b.tscore.3', 'frailty.fu.3', 'frailty.fu.12'
)
...
if(rerun.imputed){
  ...
  create.areg.obj <- function(cohort.data, marker.measure){
    areg.formula <-
      as.formula(paste0('~ ',
                        paste(c('age.enroll', 'sex.pp', 'charlson.score', 'edu',
                                'I(iqcode.score.e)', 'stroke.risk',
                                'mean.benz.cube', 'mean.op.cube',
                                'mean.prop.cube', 'I(mean.dex.cube)',
                                'I(mean.hal.cube)', 'icudays.sevseptic.s',
                                'del.s.imp', 'frailty.num', 'I(adl.e)',
                                'I(faq.e)',
  ...
  create.mice.predmat <- function(marker.measure = c('mean', 'first', 'pctchg')){
    ...
    cols.not.used <- unlist(lapply(meas.not.used,
                                   FUN = function(x){
                                     grep(x, names(model.data.func.3), fixed = TRUE) }))
...
## Frailty
## Delirium *not* included
covar.cfs <- "rcs(age.enroll, 3) + sex.pp + rcs(charlson.score, 3) + rcs(edu, 3) + rcs(frailty.num, 3) + rcs(markervar, 3)"

## Delirium included
covar.del.cfs <- "rcs(age.enroll, 3) + sex.pp + rcs(charlson.score, 3) + rcs(edu, 3) + rcs(frailty.num, 3) + rcs(del.s.imp, 3) + rcs(markervar, 3)"
...
## cfs.covar: string representing righthand side of model spec for frailty outcome
   ## all the above should include "markervar" for gsubbing actual biomarker variable names
...
run.marker.mods <- function(marker,
  ...
  cfs.covar.lin <- gsub('rcs\\(', '', gsub(', [0-9]+\\)', '', cfs.covar))
  ...
  ## Mean vs. CFS
  mean.cfs3.form <-
    as.formula(make.modform('frailty.fu.3', transmarker.mean, cfs.covar))
  mean.cfs.nowt.3 <-
    fit.mult.impute(mean.cfs3.form,
                    fitter = ols, xtrans = mice.mean.3, data = model.data.func.3)
  mean.cfs.3.pnl <- anova(mean.cfs.nowt.3)['TOTAL NONLINEAR', 'P']
  
  if(mean.cfs.3.pnl > pcutoff){
    mean.cfs3.form <-
      as.formula(make.modform('frailty.fu.3', transmarker.mean, cfs.covar.lin))
    mean.cfs.nowt.3 <-
      fit.mult.impute(mean.cfs3.form,
                      fitter = ols, xtrans = mice.mean.3, data = model.data.func.3)
  }
  ...
  mean.cfs.wt.3 <-
    fit.mult.impute(mean.cfs3.form,
                    fitter = ols.fix, xtrans = mice.mean.3, weights = wt.func.3,
                    data = model.data.func.3)

  mean.cfs12.form <-
    as.formula(make.modform('frailty.fu.12', transmarker.mean, cfs.covar))
  mean.cfs.nowt.12 <-
    fit.mult.impute(mean.cfs12.form,
                    fitter = ols, xtrans = mice.mean.12, data = model.data.func.12)
  
  mean.cfs.12.pnl <- anova(mean.cfs.nowt.12)['TOTAL NONLINEAR', 'P']
  
  if(mean.cfs.12.pnl > pcutoff){
    mean.cfs12.form <-
      as.formula(make.modform('frailty.fu.12', transmarker.mean, cfs.covar.lin))
    mean.cfs.nowt.3 <-
      fit.mult.impute(mean.cfs12.form,
                      fitter = ols, xtrans = mice.mean.12, data = model.data.func.12)
  }
  ...
  mean.cfs.wt.12 <-
    fit.mult.impute(mean.cfs12.form,
                    fitter = ols.fix, xtrans = mice.mean.12, weights = wt.func.12,
                    data = model.data.func.12)

  ## First vs. CFS
  first.cfs3.form <-
    as.formula(make.modform('frailty.fu.3', transmarker.first, cfs.covar))
  first.cfs.nowt.3 <-
    fit.mult.impute(first.cfs3.form,
                    fitter = ols, xtrans = mice.first.3, data = model.data.func.3)

  first.cfs.3.pnl <- anova(first.cfs.nowt.3)['TOTAL NONLINEAR', 'P']
  
  if(first.cfs.3.pnl > pcutoff){
    first.cfs3.form <-
      as.formula(make.modform('frailty.fu.3', transmarker.first, cfs.covar.lin))
    first.cfs.nowt.3 <-
      fit.mult.impute(first.cfs3.form,
                      fitter = ols, xtrans = mice.first.3, data = model.data.func.3)
  }
  ...
  first.cfs.wt.3 <-
    fit.mult.impute(first.cfs3.form,
                    fitter = ols.fix, xtrans = mice.first.3, weights = wt.func.3,
                    data = model.data.func.3)

  first.cfs12.form <-
    as.formula(make.modform('frailty.fu.12', transmarker.first, cfs.covar))
  first.cfs.nowt.12 <-
    fit.mult.impute(first.cfs12.form,
                    fitter = ols, xtrans = mice.first.12, data = model.data.func.12)

  first.cfs.12.pnl <- anova(first.cfs.nowt.12)['TOTAL NONLINEAR', 'P']
  
  if(first.cfs.12.pnl > pcutoff){
    first.cfs12.form <-
      as.formula(make.modform('frailty.fu.12', transmarker.first, cfs.covar.lin))
    first.cfs.nowt.12 <-
      fit.mult.impute(first.cfs12.form,
                      fitter = ols, xtrans = mice.first.12, data = model.data.func.12)
  }
  ...
  first.cfs.wt.12 <-
    fit.mult.impute(first.cfs12.form,
                    fitter = ols.fix, xtrans = mice.first.12,
                    weights = wt.func.12, data = model.data.func.12)

  ## % change vs. CFS
  pctchg.cfs3.form <-
    as.formula(make.modform('frailty.fu.3', transmarker.pctchg, cfs.covar))
  pctchg.cfs.nowt.3 <-
    fit.mult.impute(pctchg.cfs3.form, fitter = ols, xtrans = mice.pctchg.3,
                    data = model.data.pctchg.func.3)

  pctchg.cfs.3.pnl <- anova(pctchg.cfs.nowt.3)['TOTAL NONLINEAR', 'P']
  
  if(pctchg.cfs.3.pnl > pcutoff){
    pctchg.cfs3.form <-
      as.formula(make.modform('frailty.fu.3', transmarker.pctchg, cfs.covar.lin))
    pctchg.cfs.nowt.3 <-
      fit.mult.impute(pctchg.cfs3.form,
                      fitter = ols, xtrans = mice.pctchg.3, data = model.data.func.3)
  }
    
  pctchg.cfs.wt.3 <-
    fit.mult.impute(pctchg.cfs3.form, fitter = ols.fix, xtrans = mice.pctchg.3,
                    weights = wt.func.3, data = model.data.pctchg.func.3)

  pctchg.cfs12.form <-
    as.formula(make.modform('frailty.fu.12', transmarker.pctchg, cfs.covar))
  pctchg.cfs.nowt.12 <-
    fit.mult.impute(pctchg.cfs12.form, fitter = ols, xtrans = mice.pctchg.12,
                    data = model.data.pctchg.func.12)

  pctchg.cfs.12.pnl <- anova(pctchg.cfs.nowt.12)['TOTAL NONLINEAR', 'P']
  
  if(pctchg.cfs.12.pnl > pcutoff){
    pctchg.cfs12.form <-
      as.formula(make.modform('frailty.fu.12', transmarker.pctchg, cfs.covar.lin))
    pctchg.cfs.nowt.12 <-
      fit.mult.impute(pctchg.cfs12.form,
                      fitter = ols, xtrans = mice.pctchg.12, data = model.data.func.12)
  }
    
  pctchg.cfs.wt.12 <-
    fit.mult.impute(pctchg.cfs12.form, fitter = ols.fix, xtrans = mice.pctchg.12,
                    weights = wt.func.12, data = model.data.pctchg.func.12)
  ...
  return(list('mean.rbans.nowt.3' = mean.rbans.nowt.3,
              ...
              'mean.cfs.nowt.3' = mean.cfs.nowt.3,
              'mean.cfs.wt.3' = mean.cfs.wt.3,
              'mean.cfs.nowt.12' = mean.cfs.nowt.12,
              'mean.cfs.wt.12' = mean.cfs.wt.12,
              'first.cfs.nowt.3' = first.cfs.nowt.3,
              'first.cfs.wt.3' = first.cfs.wt.3,
              'first.cfs.nowt.12' = first.cfs.nowt.12,
              'first.cfs.wt.12' = first.cfs.wt.12,
              'pctchg.cfs.nowt.3' = pctchg.cfs.nowt.3,
              'pctchg.cfs.wt.3' = pctchg.cfs.wt.3,
              'pctchg.cfs.nowt.12' = pctchg.cfs.nowt.12,
              'pctchg.cfs.wt.12' = pctchg.cfs.wt.12,
              ...
              'mean.cfs.3.pnl' = mean.cfs.3.pnl,
              'mean.cfs.12.pnl' = mean.cfs.12.pnl,
              'first.cfs.3.pnl' = first.cfs.3.pnl,
              'first.cfs.12.pnl' = first.cfs.12.pnl,
              'pctchg.cfs.3.pnl' = pctchg.cfs.3.pnl,
              'pctchg.cfs.12.pnl' = pctchg.cfs.12.pnl))
}
...
# check.diag.linear(crp.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(crp.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
...
# check.diag.linear(ifngamma.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(ifngamma.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## IL-10
...
# check.diag.linear(il10.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(il10.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## IL-12
...
# check.diag.linear(il12.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(il12.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## IL-1b
...
# check.diag.linear(il1b.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(il1b.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## IL-6
...
# check.diag.linear(il6.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(il6.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## IL-8
...
# check.diag.linear(il8.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(il8.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## MMP9
...
# check.diag.linear(mmp9.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(mmp9.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## Protein C
...
# check.diag.linear(proteinc.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(proteinc.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## TNF-alpha
...
# check.diag.linear(tnfa.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(tnfa.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## TNFR1
...
# check.diag.linear(tnfr1.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(tnfr1.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
...
# check.diag.linear(crp.del.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(crp.del.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## IFN-gamma
...
# check.diag.linear(ifngamma.del.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(ifngamma.del.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## IL-10
...
# check.diag.linear(il10.del.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(il10.del.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## IL-12
...
# check.diag.linear(il12.del.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(il12.del.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## IL-1b
...
# check.diag.linear(il1b.del.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(il1b.del.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## IL-6
...
# check.diag.linear(il6.del.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(il6.del.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## IL-8
...
# check.diag.linear(il8.del.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(il8.del.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## MMP9
...
# check.diag.linear(mmp9.del.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(mmp9.del.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## Protein C
...
# check.diag.linear(proteinc.del.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(proteinc.del.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## TNF-alpha
...
# check.diag.linear(tnfa.del.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(tnfa.del.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
#
# ## TNFR1
...
# check.diag.linear(tnfr1.del.mods, time = 3, out.name = 'cfs', out.str = 'CFS')
# check.diag.linear(tnfr1.del.mods, time = 12, out.name = 'cfs', out.str = 'CFS')
...
bio.designmats.3 <-
  lapply(biomarker.measures, FUN = create.func.designmat, cohort.var = 'func.cohort.3')
bio.designmats.12 <-
  lapply(biomarker.measures, FUN = create.func.designmat, cohort.var = 'func.cohort.12')
names(bio.designmats.3) <- names(bio.designmats.12) <- biomarker.measures
...
calc.nbresults <- function(nbobj,          ## negbin model object (from glm.nb())
  ...
  if(sum(!(coefnames %in% colnames(designmat))) > 0){
    stop("Variables in nbobj are not in designmat")
  }
  
  if(!(xvar %in% colnames(designmat))){
    stop("X axis variable name not in column names of design matrix")
  }
  
  designmat <- designmat[,coefnames]
...
## -- Function to create data set for plotting all biomarker measures vs a specific test at --------
## -- both times, weighted and unweighted ----------------------------------------------------------
create.func.plotdata <- function(marker,
                                 marker.mods,
                                 test = c('adl', 'faq')){
  
  ## Get names of all model objects related to test
  test.mods <- unlist(lapply(c('mean', 'first', 'pctchg'),
                             FUN = function(m){
                               grep(paste0('^', m, '\\.', test, '\\.[no]*wt\\.[0-9]+$'),
                                    names(marker.mods), value = TRUE) }))
  
  ## Function to create data set for a single combo of marker, measure, test, weightedness, time
  single.plot.data <- function(test.mod){
    ## Setup: split name of model into pertinent info
    mod.info <- unlist(strsplit(test.mod, '.', fixed = TRUE))
    names(mod.info) <- c('measure', 'test', 'weighted', 'time')
    
    ## Choose design matrix list to use
    if(mod.info['time'] == 3){
      use.design <- bio.designmats.3
    } else{
      use.design <- bio.designmats.12
    }

    ## Determine name of X variable to use    
    xvar <- paste(marker, mod.info['measure'], sep = '.')
    if(mod.info['measure'] %in% c('mean', 'first')){
      xvar <- paste(xvar, 't', sep = '.')
    }
    
    ## Calculate predicted values/CIs
    plotdata <- calc.nbresults(nbobj = marker.mods[[test.mod]],
                               designmat = use.design[[xvar]],
                               xvar = xvar)
    
    ## Add measure (mean, first, %), time, weighted/unweighted to data set to use for faceting
    plotdata$measure <- mod.info['measure']
    plotdata$time <- mod.info['time']
    plotdata$weighted <- mod.info['weighted']
    plotdata
  }

  ## Create a single data set of all test-related results
  test.plotdata <- do.call(rbind, lapply(test.mods, FUN = single.plot.data))
  
  ## Create factor versions of faceting variables
  test.plotdata$measure.f <- with(test.plotdata, factor(ifelse(measure == 'mean', 1,
                                                        ifelse(measure == 'first', 2,
                                                        ifelse(measure == 'pctchg', 3, NA))),
                                                        levels = 1:3,
                                                        labels = c('Log10(Mean)',
                                                                   'Log10(First)',
                                                                   '% Change')))
  test.plotdata$weighted.f <- with(test.plotdata, factor(ifelse(weighted == 'nowt', 1,
                                                         ifelse(weighted == 'wt', 2, NA)),
                                                         levels = 1:2,
                                                         labels = c('Unweighted', 'Weighted')))
  test.plotdata$time.f <- with(test.plotdata, factor(ifelse(time == 3, 1, 2),
                                                     levels = 1:2,
                                                     labels = c('3 Months', '12 Months')))
  test.plotdata$marker <- marker
  test.plotdata$test <- test
...
calc.ratioci.inflltci <- function(marker,                  ## string, eg 'crp'
                                  modlist,                 ## list of model fits
                                  modobj,                  ## model object
                                  marker.adjust = NULL,    ## Adjustment values for main term;
                                                           ## default = 75th vs 25th percentile
                                  markerp.adjust = NULL,   ## Adjustment values for nonlinear term
                                  cohort.specific = FALSE, ## For defaults, use only patients with
                                                           ##  fxnal outcomes at spec time point?
                                  alpha = 0.05){           ## % CIs
  
  if(!(modobj %in% names(modlist))){
    stop('modobj is not an element of modlist')
  }
  
  ## Split name of model object to get model info
  modinfo <- unlist(strsplit(modobj, '.', fixed = TRUE))
  names(modinfo) <- c('measure', 'test', 'weighted', 'time')
  
  time <- as.numeric(tail(unlist(strsplit(modobj, '.', fixed = TRUE)), n = 1))
  if(is.na(time)){
    stop('modobj name appears to be incorrect')
  }
  
  ## Data set to use: if cohort.specific = T, all patients in cohort *at specific time point*;
  ##  otw, all patients in original cohort
  if(cohort.specific){
    usedata <- model.data[model.data[,paste0('func.cohort.', time)],]
  } else{
    usedata <- model.data
  }

  ## Get name of X variable  
  xvar <- paste0(marker, '.', modinfo['measure'])
  if(modinfo['measure'] %in% c('mean', 'first')){
    xvar <- paste0(xvar, '.t')
  }
  
  ## Get model coefficients, vcov matrix
  modcoefs <- pool(modlist[[modobj]])$qbar
  modvcov <- pool(modlist[[modobj]])$t
  
  ## Determine what values to adjust biomarker terms to
  ## (by default, 75th and 25th percentiles of time-specific data set)
  if(is.null(marker.adjust)){
    marker.adjust <- as.numeric(quantile(usedata[,xvar], probs = c(0.25, 0.75), na.rm = TRUE))
  }
  
  ## If nonlinear term is included and no defaults given, get values to adjust nonlinear term to
  if(is.null(markerp.adjust) & paste0(xvar, '.p') %in% names(modcoefs)){
    marker.knots <- rcspline.eval(usedata[,xvar], nk = 3, knots.only = TRUE)
    markerp.adjust <- as.numeric(rcspline.eval(marker.adjust, knots = marker.knots))
  }
  ...
  ratiovars <- lapply(1:2, FUN = function(m){
    tmp <- c('marker' = marker.adjust[m],
             'marker.p' = markerp.adjust[m])
    names(tmp) <- gsub('marker', xvar, names(tmp))
    tmp
  })
  ...
  return(c('pointest' = ratiovar.or, 'lcl' = ratiovar.lcl, 'ucl' = ratiovar.ucl,
           'ref.val' = marker.adjust[1], 'comp.val' = marker.adjust[2]))
}
...
create.results.table <- function(modlist, ## List of model objects (eg, crp.mods)
  ...
  if(test %in% c('rbans', 'trailb', 'cfs')){
    ...
    results.data <-
      as.data.frame(do.call(rbind,
                            lapply(grep(paste0('\\.', test, '\\.[no]*wt\\.[0-9]+$'),
                                        names(modlist),
                                        value = TRUE),
                                   FUN = function(x){
                                     mod.n <- ifelse(test %in% c('rbans', 'trailb', 'cfs'),
                                                     modlist[[x]]$stats['n'],
                                                     nrow(modlist[[x]]$analyses[[1]]$model))
                                     mod.pnl <- modlist[[paste0(gsub('[no]*wt\\.', '', x), '.pnl')]]
      ...
      as.data.frame(do.call(rbind,
                            lapply(grep(paste0('\\.', test, '\\.[no]*wt\\.[0-9]+$'),
                                   ...
                                   FUN = function(x){
                                     ...
                                     mod.peci <-
                                       calc.ratioci.inflltci(marker = marker,
                                                             modlist = modlist,
                                                             modobj = x)
  ...
  pnl.data <- results.data %>%
    dplyr::select(which.mod, p.nonlin) %>%
    separate(which.mod, into = c('measure', 'test', 'weight', 'time')) %>%
    mutate(measure.str = ifelse(measure == 'first', 'Log10(First)',
                         ifelse(measure == 'mean', 'Log10(Mean)', '\\% Change'))) %>%
    dplyr::select(-weight) %>%
    unique() %>%
    spread(key = time, value = p.nonlin)
  names(pnl.data)[-3] <- paste0('pnl.', names(pnl.data)[-3])

  results.data <- results.data %>%
    dplyr::select(-n, -p.nonlin) %>%
    separate(which.mod, into = c('measure', 'test', 'weight', 'time')) %>%
    mutate(measure.str = ifelse(measure == 'first', 'Log10(First)',
                         ifelse(measure == 'mean', 'Log10(Mean)', '\\% Change')),
           low = rndformat(low),
    ...
    left_join(dplyr::select(pnl.data, measure.str, pnl.3, pnl.12), by = 'measure.str') %>%
    mutate(pnl.3 = formatp(pnl.3), pnl.12 = formatp(pnl.12),
           n.pnl.3 = paste(n.3, pnl.3, sep = '; '),
           n.pnl.12 = paste(n.12, pnl.12, sep = '; '))

  results.data[,c('measure.str', 'low', 'high',
                  'n.pnl.3', 'nowt.3', 'wt.3',
                  'n.pnl.12', 'nowt.12', 'wt.12')]
}

## -- Get tables for all model results -------------------------------------------------------------
## Without delirium
...
crp.cfs.results <- create.results.table(modlist = crp.mods, marker = 'crp', 'cfs')
...
ifngamma.faq.results <- create.results.table(modlist = ifngamma.mods, marker = 'ifngamma', 'faq',
                                             pval.list = ifngamma.func.pvals)
ifngamma.cfs.results <- create.results.table(modlist = ifngamma.mods, marker = 'ifngamma', 'cfs')
...
il10.faq.results <- create.results.table(modlist = il10.mods, marker = 'il10', 'faq',
                                         pval.list = il10.func.pvals)
il10.cfs.results <- create.results.table(modlist = il10.mods, marker = 'il10', 'cfs')
...
il12.faq.results <- create.results.table(modlist = il12.mods, marker = 'il12', 'faq',
                                         pval.list = il12.func.pvals)
il12.cfs.results <- create.results.table(modlist = il12.mods, marker = 'il12', 'cfs')
...
il1b.faq.results <- create.results.table(modlist = il1b.mods, marker = 'il1b', 'faq',
                                         pval.list = il1b.func.pvals)
il1b.cfs.results <- create.results.table(modlist = il1b.mods, marker = 'il1b', 'cfs')
...
il6.faq.results <- create.results.table(modlist = il6.mods, marker = 'il6', 'faq',
                                        pval.list = il6.func.pvals)
il6.cfs.results <- create.results.table(modlist = il6.mods, marker = 'il6', 'cfs')
...
il8.faq.results <- create.results.table(modlist = il8.mods, marker = 'il8', 'faq',
                                        pval.list = il8.func.pvals)
il8.cfs.results <- create.results.table(modlist = il8.mods, marker = 'il8', 'cfs')
...
mmp9.faq.results <- create.results.table(modlist = mmp9.mods, marker = 'mmp9', 'faq',
                                         pval.list = mmp9.func.pvals)
mmp9.cfs.results <- create.results.table(modlist = mmp9.mods, marker = 'mmp9', 'cfs')
...
proteinc.faq.results <- create.results.table(modlist = proteinc.mods, marker = 'proteinc', 'faq',
                                             pval.list = proteinc.func.pvals)
proteinc.cfs.results <-
  create.results.table(modlist = proteinc.mods, marker = 'proteinc', 'cfs')
...
tnfa.faq.results <- create.results.table(modlist = tnfa.mods, marker = 'tnfa', 'faq',
                                         pval.list = tnfa.func.pvals)
tnfa.cfs.results <- create.results.table(modlist = tnfa.mods, marker = 'tnfa', 'cfs')
...
tnfr1.faq.results <- create.results.table(modlist = tnfr1.mods, marker = 'tnfr1', 'faq',
                                          pval.list = tnfr1.func.pvals)
tnfr1.cfs.results <- create.results.table(modlist = tnfr1.mods, marker = 'tnfr1', 'cfs')
...
crp.del.faq.results <- create.results.table(modlist = crp.del.mods, marker = 'crp', 'faq',
                                            pval.list = crp.del.func.pvals)
crp.del.cfs.results <- create.results.table(modlist = crp.del.mods, marker = 'crp', 'cfs')
...
ifngamma.del.cfs.results <-
  create.results.table(modlist = ifngamma.del.mods, marker = 'ifngamma', 'cfs')
...
il10.del.faq.results <- create.results.table(modlist = il10.del.mods, marker = 'il10', 'faq',
                                             pval.list = il10.del.func.pvals)
il10.del.cfs.results <- create.results.table(modlist = il10.del.mods, marker = 'il10', 'cfs')
...
il12.del.faq.results <- create.results.table(modlist = il12.del.mods, marker = 'il12', 'faq',
                                             pval.list = il12.del.func.pvals)
il12.del.cfs.results <- create.results.table(modlist = il12.del.mods, marker = 'il12', 'cfs')
...
il1b.del.faq.results <- create.results.table(modlist = il1b.del.mods, marker = 'il1b', 'faq',
                                             pval.list = il1b.del.func.pvals)
il1b.del.cfs.results <- create.results.table(modlist = il1b.del.mods, marker = 'il1b', 'cfs')
...
il6.del.faq.results <- create.results.table(modlist = il6.del.mods, marker = 'il6', 'faq',
                                            pval.list = il6.del.func.pvals)
il6.del.cfs.results <- create.results.table(modlist = il6.del.mods, marker = 'il6', 'cfs')
...
il8.del.faq.results <- create.results.table(modlist = il8.del.mods, marker = 'il8', 'faq',
                                            pval.list = il8.del.func.pvals)
il8.del.cfs.results <- create.results.table(modlist = il8.del.mods, marker = 'il8', 'cfs')
...
mmp9.del.faq.results <- create.results.table(modlist = mmp9.del.mods, marker = 'mmp9', 'faq',
                                             pval.list = mmp9.del.func.pvals)
mmp9.del.cfs.results <- create.results.table(modlist = mmp9.del.mods, marker = 'mmp9', 'cfs')
...
proteinc.del.cfs.results <-
  create.results.table(modlist = proteinc.del.mods, marker = 'proteinc', 'cfs')
...
tnfa.del.faq.results <- create.results.table(modlist = tnfa.del.mods, marker = 'tnfa', 'faq',
                                             pval.list = tnfa.del.func.pvals)
tnfa.del.cfs.results <- create.results.table(modlist = tnfa.del.mods, marker = 'tnfa', 'cfs')
...
tnfr1.del.faq.results <- create.results.table(modlist = tnfr1.del.mods, marker = 'tnfr1', 'faq',
                                              pval.list = tnfr1.del.func.pvals)
tnfr1.del.cfs.results <- create.results.table(modlist = tnfr1.del.mods, marker = 'tnfr1', 'cfs')


## -- Function to print results tables in Latex ----------------------------------------------------
latex.results <- function(resultsdata, markerstr, teststr){
  latex(resultsdata, file = '',
        colheads = c('', 'Ref.', 'Comp.', rep(c('N; P, NL', 'No Weights', 'Weights'), 2)),
        ...
        insert.bottom = '"P, NL": P-value for nonlinearity. If P \\textgreater 0.20, nonlinear terms were removed from the model for parsimony.')
}
...
cogresults.plot <- function(modlist, ## List of model objects (eg, crp.mods)
  ...
  pred.data <- bind_rows(lapply(grep(paste0('\\.', test, '\\.[no]*wt\\.[0-9]+$'), names(modlist)),
                                FUN = function(x){
...
## Cognitive outcomes (linear models)
funcresults.plot <- function(modlist, ## List of model objects (eg, crp.mods)
                             test = c('adl', 'faq'), ## Test of interest
                             marker, ## String; biomarker prefix (eg, 'crp')
                             xlab.addon = NULL){ ## Text added to X label ("Models W/O Delirium")

  ## Get data set of predicted values
  pred.data <- create.func.plotdata(marker = marker, marker.mods = modlist, test = test)
  ...
  ## Plot, with biomarker measures as columns and time points as rows
  marker.plot <- ggplot(aes(x = xvalue), data = pred.data) +
    facet_grid(time.f ~ measure.f, scales = 'free_x') +
    geom_ribbon(aes(ymin = count.lcl, ymax = count.ucl, fill = weighted.f), alpha = 0.4) +
    geom_line(aes(y = count.pe, colour = weighted.f)) +
    scale_colour_discrete(name = '', labels = c('Unweighted', 'Weighted')) +
    scale_fill_discrete(name = '', labels = c('Unweighted', 'Weighted')) +
    xlab(use.xlab) +
    ylab(test.ylabs[match(test, test.ylabs$testvar), 'testylab']) +
...
multiplot(funcresults.plot(modlist = crp.mods, test = 'adl', marker = 'crp',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = crp.del.mods, test = 'adl', marker = 'crp',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = crp.mods, test = 'faq', marker = 'crp',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = crp.del.mods, test = 'faq', marker = 'crp',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = ifngamma.mods, test = 'adl', marker = 'ifngamma',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = ifngamma.del.mods, test = 'adl', marker = 'ifngamma',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = ifngamma.mods, test = 'faq', marker = 'ifngamma',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = ifngamma.del.mods, test = 'faq', marker = 'ifngamma',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = il10.mods, test = 'adl', marker = 'il10',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = il10.del.mods, test = 'adl', marker = 'il10',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = il10.mods, test = 'faq', marker = 'il10',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = il10.del.mods, test = 'faq', marker = 'il10',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = il12.mods, test = 'adl', marker = 'il12',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = il12.del.mods, test = 'adl', marker = 'il12',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = il12.mods, test = 'faq', marker = 'il12',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = il12.del.mods, test = 'faq', marker = 'il12',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = il1b.mods, test = 'adl', marker = 'il1b',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = il1b.del.mods, test = 'adl', marker = 'il1b',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = il1b.mods, test = 'faq', marker = 'il1b',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = il1b.del.mods, test = 'faq', marker = 'il1b',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = il6.mods, test = 'adl', marker = 'il6',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = il6.del.mods, test = 'adl', marker = 'il6',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = il6.mods, test = 'faq', marker = 'il6',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = il6.del.mods, test = 'faq', marker = 'il6',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = il8.mods, test = 'adl', marker = 'il8',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = il8.del.mods, test = 'adl', marker = 'il8',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = il8.mods, test = 'faq', marker = 'il8',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = il8.del.mods, test = 'faq', marker = 'il8',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = mmp9.mods, test = 'adl', marker = 'mmp9',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = mmp9.del.mods, test = 'adl', marker = 'mmp9',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = mmp9.mods, test = 'faq', marker = 'mmp9',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = mmp9.del.mods, test = 'faq', marker = 'mmp9',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = proteinc.mods, test = 'adl', marker = 'proteinc',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = proteinc.del.mods, test = 'adl', marker = 'proteinc',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = proteinc.mods, test = 'faq', marker = 'proteinc',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = proteinc.del.mods, test = 'faq', marker = 'proteinc',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = tnfa.mods, test = 'adl', marker = 'tnfa',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = tnfa.del.mods, test = 'adl', marker = 'tnfa',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = tnfa.mods, test = 'faq', marker = 'tnfa',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = tnfa.del.mods, test = 'faq', marker = 'tnfa',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = tnfr1.mods, test = 'adl', marker = 'tnfr1',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = tnfr1.del.mods, test = 'adl', marker = 'tnfr1',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
multiplot(funcresults.plot(modlist = tnfr1.mods, test = 'faq', marker = 'tnfr1',
                           xlab.addon = 'Models Without Delirium'),
          funcresults.plot(modlist = tnfr1.del.mods, test = 'faq', marker = 'tnfr1',
                           xlab.addon = 'Models With Delirium'),
          layout = matrix(1:2, ncol = 1))
...
\clearpage
\section{Biomarkers vs. CSHA Clinical Frailty Score}
We used linear regression to examine the relationships between biomarkers, measured three different
ways, and CSHA Clinical Frailty Scores at 3- and 12-month followup. Summary results for each
association follow, with detailed model results to come. Point esimates (confidence limits)
represent the adjusted difference in test scores for a patient at the 75th vs. the 25th percentile
of the biomarker measure, assuming all other covariates are the same.
...
cohorts to patients with complete biomarker data, we remove the need to impute biomarker data, but
we do use multiple imputation to impute other missing covariates (education was the only covariate
with missing data) and to impute missing frailty data. Multiple imputation helps reduce bias; in
our case, bias would likely come from the sickest patients (ie, most likely to score poorly) not
completing all functional testing.

All models included the following covariates; all continuous variables (including biomarker) were allowed to have a nonlinear association with the outcome using restricted cubic splines. Mean and first biomarker were transformed by taking the base 10 logarithm, so that extremely high values did not cause
problems with model convergence.
...
\item Charlson score
\item Years of education
\item Clinical frailty score at enrollment
\end{itemize}

Previous analyses showed no meaningful association between frailty and delirium; therefore, we do not present models investigating mediation with delirium included for this outcome.

\clearpage
\subsection{CRP}
<<printcrpcfs, results='asis', fig.cap = 'CRP vs. Clinical Frailty Scores', fig.align = 'center', fig.width = 10, fig.height = 4.5, fig.pos = '!h'>>=
latex.results(crp.cfs.results, 'CRP', 'Clinical Frailty Scores')
cogresults.plot(modlist = crp.mods, test = 'cfs', marker = 'crp', xlab.addon = 'Adjusted for Covariates')

@

\clearpage
\subsection{IFN-$\gamma$}
<<printifngammacfs, results='asis', fig.cap = 'IFN-gamma vs. Clinical Frailty Scores', fig.align = 'center', fig.width = 10, fig.height = 4.5, fig.pos = '!h'>>=
latex.results(ifngamma.cfs.results, 'IFN-gamma', 'Clinical Frailty Scores')
cogresults.plot(modlist = ifngamma.mods, test = 'cfs', marker = 'ifngamma', xlab.addon = 'Adjusted for Covariates')

@

\clearpage
\subsection{IL-10}
<<printil10cfs, results='asis', fig.cap = 'IL-10 vs. Clinical Frailty Scores', fig.align = 'center', fig.width = 10, fig.height = 4.5, fig.pos = '!h'>>=
latex.results(il10.cfs.results, 'IL-10', 'Clinical Frailty Scores')
cogresults.plot(modlist = il10.mods, test = 'cfs', marker = 'il10', xlab.addon = 'Adjusted for Covariates')

@

\clearpage
\subsection{IL-12}
<<printil12cfs, results='asis', fig.cap = 'IL-12 vs. Clinical Frailty Scores', fig.align = 'center', fig.width = 10, fig.height = 4.5, fig.pos = '!h'>>=
latex.results(il12.cfs.results, 'IL-12', 'Clinical Frailty Scores')
cogresults.plot(modlist = il12.mods, test = 'cfs', marker = 'il12', xlab.addon = 'Adjusted for Covariates')

@

\clearpage
\subsection{IL-1$\beta$}
<<printil1bcfs, results='asis', fig.cap = 'IL-1b vs. Clinical Frailty Scores', fig.align = 'center', fig.width = 10, fig.height = 4.5, fig.pos = '!h'>>=
latex.results(il1b.cfs.results, 'IL-1b', 'Clinical Frailty Scores')
cogresults.plot(modlist = il1b.mods, test = 'cfs', marker = 'il1b', xlab.addon = 'Adjusted for Covariates')

@

\clearpage
\subsection{IL-6}
<<printil6cfs, results='asis', fig.cap = 'IL-6 vs. Clinical Frailty Scores', fig.align = 'center', fig.width = 10, fig.height = 4.5, fig.pos = '!h'>>=
latex.results(il6.cfs.results, 'IL-6', 'Clinical Frailty Scores')
cogresults.plot(modlist = il6.mods, test = 'cfs', marker = 'il6', xlab.addon = 'Adjusted for Covariates')

@

\clearpage
\subsection{IL-8}
<<printil8cfs, results='asis', fig.cap = 'IL-8 vs. Clinical Frailty Scores', fig.align = 'center', fig.width = 10, fig.height = 4.5, fig.pos = '!h'>>=
latex.results(il8.cfs.results, 'IL-8', 'Clinical Frailty Scores')
cogresults.plot(modlist = il8.mods, test = 'cfs', marker = 'il8', xlab.addon = 'Adjusted for Covariates')

@

\clearpage
\subsection{MMP9}
<<printmmp9cfs, results='asis', fig.cap = 'MMP9 vs. Clinical Frailty Scores', fig.align = 'center', fig.width = 10, fig.height = 4.5, fig.pos = '!h'>>=
latex.results(mmp9.cfs.results, 'MMP9', 'Clinical Frailty Scores')
cogresults.plot(modlist = mmp9.mods, test = 'cfs', marker = 'mmp9', xlab.addon = 'Adjusted for Covariates')

@

\clearpage
\subsection{Protein C}
<<printproteinccfs, results='asis', fig.cap = 'Protein C vs. Clinical Frailty Scores', fig.align = 'center', fig.width = 10, fig.height = 4.5, fig.pos = '!h'>>=
latex.results(proteinc.cfs.results, 'Protein C', 'Clinical Frailty Scores')
cogresults.plot(modlist = proteinc.mods, test = 'cfs', marker = 'proteinc', xlab.addon = 'Adjusted for Covariates')

@

\clearpage
\subsection{TNF$\alpha$}
<<printtnfacfs, results='asis', fig.cap = 'TNF-alpha vs. Clinical Frailty Scores', fig.align = 'center', fig.width = 10, fig.height = 4.5, fig.pos = '!h'>>=
latex.results(tnfa.cfs.results, 'TNF-alpha', 'Clinical Frailty Scores')
cogresults.plot(modlist = tnfa.mods, test = 'cfs', marker = 'tnfa', xlab.addon = 'Adjusted for Covariates')

@

\clearpage
\subsection{TNFR1}
<<printtnfr1cfs, results='asis', fig.cap = 'TNFR1 vs. Clinical Frailty Scores', fig.align = 'center', fig.width = 10, fig.height = 4.5, fig.pos = '!h'>>=
latex.results(tnfr1.cfs.results, 'TNFR1', 'Clinical Frailty Scores')
cogresults.plot(modlist = tnfr1.mods, test = 'cfs', marker = 'tnfr1', xlab.addon = 'Adjusted for Covariates')

@

%% -- Code for publication figures ---------------------------------------------
% 
<<forestplot>>=
## -- Create data.frame of all model results at all time points ----------------
forest_results <- map2_df(
  .x = rep(gsub("\\.imp$", "", biomarker.vnames), 4),
  .y = rep(c("rbans", "trailb", "adl", "faq"), each = length(biomarker.vnames)),
  ~ get(paste(.x, .y, "results", sep = ".")) %>% mutate(marker = .x, test = .y)
) %>%
  ## Only need mean biomarker rows, estimates/CIs
  filter(measure.str == "Log10(Mean)") %>%
  dplyr::select(marker, test, starts_with("wt")) %>%
  ## Reshape so that each time point is a separate line
  gather(key = timept, value = est_ci, starts_with("wt")) %>%
  ## Extract estimate, confidence limits, p-values from character string
  separate(
    est_ci, into = c("est", "lcl", "ucl", "pval"), sep = " \\(|, |\\); P = "
  ) %>%
  ## Turn estimate, CIs, p into numeric
  mutate_at(vars(-marker, -test, -timept), as.numeric) %>%
  mutate(
    ## Add reference values, formatting
    timept = factor(
      ifelse(timept == "wt.3", 2, 1),
      levels = 1:2, labels = c("12 Months", "3 Months")
    ),
    refline = ifelse(test %in% c("rbans", "trailb"), 0, 1),
    test = factor(
      case_when(
        test == "rbans" ~ 1,
        test == "trailb" ~ 2,
      ...
      levels = 1:4,
      labels = c("Difference in RBANS\n(95% CI)",
                 "Difference in Trails B\n(95% CI)",
                 "Incidence Rate Ratio, Katz ADL \n(95% CI)",
                 "Incidence Rate Ratio, FAQ \n(95% CI)")
    ),
    marker = factor(
      case_when(
        marker == "crp" ~ 11,
      ...
      levels = 1:11,
      labels = c(
        "Protein C", "TNFR1", "TNF-a", "MMP-9", "IL-12", "IL-10", "IL-8",
        "IL-6", "IL-1b", "IFN-g", "CRP"
      )
    )
  )

## -- Wrapper functions for facet axis breaks (thanks, Jim Hester!) ------------
forest_breaks <- function(x){
  if (max(x) <= 2) seq(0.5, 1.5, 0.25) else seq(-4, 4, 1)
}

forest_plot_cog <- ggplot(
  data = forest_results %>% filter(stringr::str_detect(test, "^Difference"))
) +
  aes(y = est, x = marker, group = timept, color = timept, shape = timept) +
  facet_wrap(~ test) +
  geom_hline(aes(yintercept = refline), linetype = "dotted", color = "gray50") +
  geom_pointrange(
    aes(ymin = lcl, ymax = ucl), position = position_dodge(width = 0.7)
  ) +
  scale_colour_manual(
    values = c("#790001", "#003D74"),
    guide = guide_legend(reverse = TRUE), name = ""
  ) +
  scale_shape(guide = guide_legend(reverse = TRUE), name = "") +
  scale_y_continuous(limits = c(-4, 4.1), breaks = seq(-4, 4, 1)) +
  coord_flip() +
  theme_minimal() +
  theme(
...
forest_plot_func <- ggplot(
  data = forest_results %>% filter(stringr::str_detect(test, "^Incidence"))
) +
  aes(y = est, x = marker, group = timept, color = timept, shape = timept) +
  facet_wrap(~ test) +
  geom_hline(aes(yintercept = refline), linetype = "dotted", color = "gray50") +
  ...
  scale_shape(guide = guide_legend(reverse = TRUE), name = "") +
  scale_y_continuous(limits = c(0.5, 1.55), breaks = seq(0.5, 1.5, 0.25)) +
  coord_flip() +
  theme_minimal() +
  theme(
    legend.position = "bottom",
...
ggsave(
  ...
  multiplot(forest_plot_cog, forest_plot_func, layout = matrix(1:2, ncol = 1)),
  device = "pdf",
  width = 7, height = 10
)
