...
always.use <- c('age.enroll', 'charlson.score', 'edu', 'iqcode.score.e', 'stroke.risk',
                ...
                'adl.totscore.3', 'faq.rdscore.12', 'faq.rdscore.3', 'rbans.global.score.12',
...
## Functional outcomes
## Delirium *not* included
covar.func <- "rcs(age.enroll, 3) + rcs(charlson.score, 3) + edu + iqcode.score.e + rcs(stroke.risk, 3) + rcs(mean.benz.cube, 3) + rcs(mean.op.cube, 3) + rcs(mean.prop.cube, 3) + mean.dex.cube + mean.hal.cube + adl.e + faq.e + as.numeric(frailty) + rcs(markervar, 3) | rcs(age.enroll, 3) + rcs(charlson.score, 3) + edu + iqcode.score.e + rcs(stroke.risk, 3) + adl.e + faq.e + as.numeric(frailty) + rcs(markervar, 3)"

## Delirium included
covar.del.func <- "rcs(age.enroll, 3) + rcs(charlson.score, 3) + edu + iqcode.score.e + rcs(stroke.risk, 3) + rcs(mean.benz.cube, 3) + rcs(mean.op.cube, 3) + rcs(mean.prop.cube, 3) + mean.dex.cube + mean.hal.cube + adl.e + faq.e + as.numeric(frailty) + rcs(del.s.imp, 3) + rcs(markervar, 3) | rcs(age.enroll, 3) + rcs(charlson.score, 3) + edu + iqcode.score.e + rcs(stroke.risk, 3) + adl.e + faq.e + as.numeric(frailty) + rcs(del.s.imp, 3) + rcs(markervar, 3)"
...
run.marker.mods <- function(marker, cog.covar = covar.cog, func.covar = covar.func){
  ...
  mean.adl.nowt.3 <-
    eval(parse(text = sprintf("with(mice.mean.3, zeroinfl(formula = %s, dist = 'negbin'))",
                              mean.adl3.formula)))
  mean.adl.wt.3 <-
    eval(parse(text = sprintf("with(mice.mean.3, zeroinfl(formula = %s, dist = 'negbin', weights = wt.func.3))",
                              mean.adl3.formula)))

  mean.adl12.formula <- make.modform('adl.totscore.12', transmarker.mean, func.covar)
  mean.adl.nowt.12 <-
    eval(parse(text = sprintf("with(mice.mean.12, zeroinfl(formula = %s, dist = 'negbin'))",
                              mean.adl12.formula)))
  mean.adl.wt.12 <-
    eval(parse(text = sprintf("with(mice.mean.12, zeroinfl(formula = %s, dist = 'negbin', weights = wt.func.12))",
                              mean.adl12.formula)))

  ## First vs. ADL
  first.adl3.formula <- make.modform('adl.totscore.3', transmarker.first, func.covar)
  first.adl.nowt.3 <-
    eval(parse(text = sprintf("with(mice.first.3, zeroinfl(formula = %s, dist = 'negbin'))",
                              first.adl3.formula)))
  first.adl.wt.3 <-
    eval(parse(text = sprintf("with(mice.first.3, zeroinfl(formula = %s, dist = 'negbin', weights = wt.func.3))",
                              first.adl3.formula)))

  first.adl12.formula <- make.modform('adl.totscore.12', transmarker.first, func.covar)
  first.adl.nowt.12 <-
    eval(parse(text = sprintf("with(mice.first.12, zeroinfl(formula = %s, dist = 'negbin'))",
                              first.adl12.formula)))
  first.adl.wt.12 <-
    eval(parse(text = sprintf("with(mice.first.12, zeroinfl(formula = %s, dist = 'negbin', weights = wt.func.12))",
                              first.adl12.formula)))

  ## % change vs. ADL
  pctchg.adl3.formula <- make.modform('adl.totscore.3', transmarker.pctchg, func.covar)
  pctchg.adl.nowt.3 <-
    eval(parse(text = sprintf("with(mice.pctchg.3, zeroinfl(formula = %s, dist = 'negbin'))",
                              pctchg.adl3.formula)))
  pctchg.adl.wt.3 <-
    eval(parse(text = sprintf("with(mice.pctchg.3, zeroinfl(formula = %s, dist = 'negbin', weights = wt.func.3))",
                              pctchg.adl3.formula)))

  pctchg.adl12.formula <- make.modform('adl.totscore.12', transmarker.pctchg, func.covar)
  pctchg.adl.nowt.12 <-
    eval(parse(text = sprintf("with(mice.pctchg.12, zeroinfl(formula = %s, dist = 'negbin'))",
                              pctchg.adl12.formula)))
  pctchg.adl.wt.12 <-
    eval(parse(text = sprintf("with(mice.pctchg.12, zeroinfl(formula = %s, dist = 'negbin', weights = wt.func.12))",
                              pctchg.adl12.formula)))
