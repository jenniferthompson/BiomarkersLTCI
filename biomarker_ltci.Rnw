...
desc.vars <- c('age.enroll', 'sex.pp', 'race.pp', 'edu', 'charlson.score', 'iqcode.score.e',
               'stroke.risk', 'adl.e', 'faq.e', 'frailty', 'mean.benz.icu', 'mean.op.new.icu',
               'mean.prop.icu', 'mean.dex.icu', 'mean.hal.icu', 'ever.vent.s', 'vent.los.tot.eo',
...
desc.data.all <-
  desc.data[desc.data$id %nin%
              subset(all.oneobs, studywd.amt %in% c('2. W/D from Participation and All Data Collected',
                                                    '4. N/A Study Staff Withdrew Patient'))$id,
            desc.vars]
desc.data.all$cohort <- 1
desc.data.cog <- desc.data[desc.data$cog.cohort.either, desc.vars]
desc.data.cog$cohort <- 2
desc.data.func <- desc.data[desc.data$func.cohort.either, desc.vars]
...
latex(summaryM(formula(paste(paste(desc.vars, collapse = ' + '), '~ cohort')), data = desc.data.all),
      file = '',
      digits = 2,
      prn = FALSE,
      what = '%',
...
pred.vars <- intersect(names(crp.mods$mean.adl.nowt.3$analyses[[1]]$coefficients), names(model.data))
